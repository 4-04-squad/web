<template>
    <div id="page-login">
      ...
      <button class="login-btn" @click="login" v-else>
        se connecter
      </button>
    </div>
</template>
...
export default defineComponent({
    ...
    methods: {
      async login() {
        ...
        const redirectUrl = `https://api.intra.42.fr/oauth/authorize?client_id=u-s4t2ud-c2f546f55ed6a51dbc1fd2b0df8787814d9df1f3c58f54b363906a7523fe005d&redirect_uri=${callbackUrl}&response_type=code`
      ...
      async logout() {
        if (this.userStore.user) {
          try {
            const response = await axios.get('http://localhost:3001/auth/signout/' + this.userStore.user.id)
            .then((res) => {
